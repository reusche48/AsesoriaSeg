...
const SVG = {
    ...
    editLg:  `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>`,
    ...
    openClient: `<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><polyline points="16 11 18 13 22 9"/></svg>`,
    ...
    phoneLg: `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.48 2 2 0 0 1 3.58 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.6A16 16 0 0 0 15.4 16.09l.96-.9a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>`,
    emailLg: `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>`,
};
...
function renderClientTable(mainContainer, tableContainer, clients) {
        ...
        return `<tr>
            ...
            <td class="actions">
                <div class="actions-wrap">
                    <button type="button" class="btn-icon open-client-btn" data-id="${esc(c.id)}" title="Abrir cliente (fijar como activo)">${SVG.openClient}</button>
    ...
    const cards = clients.map((c) => {
        ...
        const gpsLink = (c.gpsLatitud && c.gpsLongitud)
            ...
            : '';
        const t22 = (val) => { const s = val ? String(val) : ''; return s.length > 22 ? `<span title="${esc(s)}">${esc(s.substring(0, 22))}…</span>` : (s ? esc(s) : '<span style="color:#bbb">—</span>'); };
        const fullName = `${c.nombreCompleto} ${c.apellidosCompletos}`;
        const phones = [c.telefono1, c.telefono2].filter(Boolean).map(t22).join(' | ') || '<span style="color:#bbb">—</span>';
        const emails = [c.email1, c.email2].filter(Boolean).map(t22).join(' | ') || '<span style="color:#bbb">—</span>';
        return `<div class="client-card">
            <div class="client-card-left">
                <div class="client-card-name">${esc(fullName)}</div>
                <div class="client-card-meta">DNI: ${esc(c.dni)} | Nac: ${c.fechaNacimiento ? esc(c.fechaNacimiento) : '<span style="color:#bbb">—</span>'}</div>
                <div class="client-card-row"><span class="client-card-icon">${SVG.phoneLg}</span>${phones}</div>
                <div class="client-card-row"><span class="client-card-icon">${SVG.emailLg}</span>${emails}</div>
            </div>
            <div class="client-card-actions">
                <button type="button" class="btn-icon open-client-btn" data-id="${esc(c.id)}" title="Abrir cliente (fijar como activo)">${SVG.openClient}</button>
                <button type="button" class="btn-icon primary edit-client-btn" data-id="${esc(c.id)}" title="Editar">${SVG.editLg}</button>
                <button type="button" class="btn-icon danger delete-client-btn" data-id="${esc(c.id)}" data-name="${esc(c.nombreCompleto)} ${esc(c.apellidosCompletos)}" title="Eliminar">${SVG.trashLg}</button>
            </div>
        </div>`;
    }).join('');
    ...
    tableContainer.querySelectorAll('.open-client-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            setActiveClient(btn.getAttribute('data-id'));
